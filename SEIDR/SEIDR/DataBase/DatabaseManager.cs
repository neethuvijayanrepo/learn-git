...
namespace SEIDR.DataBase
{
    ...
    public class DatabaseManager
    {
        ...
        public DatabaseConnection CloneConnection(string programName = null)
        {
            ...
            conn.CommandTimeout = _conn.CommandTimeout; //Command timeout is not part of connection string, need to explicitly copy this as well.
            return conn;
        }
        #endregion
        ...
        {
            get { return _conn.CommandTimeout; }
            set { _conn.CommandTimeout = value; }
        }
        /// <summary>
        /// Sets the <see cref="CommandTimeOut"/>, but only if the new value is larger than the current command timeout.
        /// </summary>
        /// <param name="NewTimeOut"></param>
        public void IncreaseCommandTimeOut(int NewTimeOut)
        {
            _conn.CommandTimeout = _conn.CommandTimeout.MaxOfComparison(NewTimeOut);
        ...
        /// <returns></returns>
        public virtual DataSet Execute(DatabaseManagerHelperModel i, bool CommitSuccess = false)
        {
            SqlConnection c = i.Connection;            
            //ToDo: Parameter store for procedure with the database connection?
            string proc = $"[{ (i.Schema ?? _Schema)}].{i.Procedure}";
        ...
        /// <returns></returns>
        public virtual DataSet Execute (string QualifiedProcedureName, object mapObj = null, bool updateMap = true)
        {
        ...
        public virtual int ExecuteNonQuery(DatabaseManagerHelperModel i, bool CommitSuccess = false)
        {
            int rc = 0;
            SqlConnection c = i.Connection;
            //ToDo: Parameter store for procedure with the database connection?
            ...
            using (c = new SqlConnection(_conn.ConnectionString))
            {
                ...
                catch(SqlException ex)
                {
                    /*
                      ...
                      SELECT *
                      FROM sys.messages 
                      WHERE Message_id IN (1205, 201, 214, 225, 349, 591, 1023, 2003)
        ...
        public virtual int ExecuteNonQuery(string QualifiedProcedureName, out int ReturnCode, object mapObj = null, bool? RetryDeadlock = null, bool updateMap = true)
        {
            CheckQualified(ref QualifiedProcedureName);
            int rv = 0;
            ReturnCode = -1;
        ...
        public virtual int ExecuteTextNonQuery(string CommandText, bool? RetryDeadlock = null)
        {
            int rv = 0;
            using (SqlConnection c = new SqlConnection(_conn.ConnectionString))
        ...
        /// <param name="RetryDeadlock">If a value is provided, determines whether to retry on a deadlock.
        /// <para>If no value is provided, will use the DatabaseManager's value, or the default for HelperModels</para></param>
        public virtual DataSet ExecuteText(string CommandText, bool? RetryDeadlock = null)
        {
            DataSet ds = new DataSet();
            using (SqlConnection c = new SqlConnection(_conn.ConnectionString))
            using (SqlCommand cmd = new SqlCommand(CommandText) { CommandType = CommandType.Text})
        ...
        /// <returns></returns>
        public virtual DataSet ExecuteText(DatabaseManagerHelperModel i, string CommandText, bool Commit = false)
        {
            DataSet ds = new DataSet();
            SqlConnection c = i.Connection;
            if(c == null)
        ...
        /// <returns></returns>
        public virtual DataTable SelectWithKey(string Key, object value, string TableOrView, int Page = -1, int PageSize = -1)
        {
        ...
        private static void CheckOutputParameters(SqlCommand cmd, object paramObj)
        {
            ...
            paramObj.GetType()
                ...
                .ForEach(a =>
                {
                    ...
                    {
                        if (a.Property.PropertyType.IsClass)
                            a.Property.SetValue( paramObj, null);
                    }
                    else
                        a.Property.SetValue(paramObj, a.Value);
                });
        }
