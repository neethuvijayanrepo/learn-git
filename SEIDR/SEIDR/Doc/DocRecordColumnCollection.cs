...
namespace SEIDR.Doc
{
    ...
    public class DocRecordColumnCollection : IEnumerable<DocRecordColumnInfo>
    {
        ...
        /// </summary>
        public DocRecordColumnCollection(string alias=null)
        {
            ...
            get
            {
                return Columns.Count > 0 
                    ...
                    && ( 
                        ...
                        !string.IsNullOrEmpty(LineEndDelimiter))
                    );
            }
        }        
        /// <summary>
        ...
        public DocRecordColumnInfo this[string SpecificAlias, string ColumnName, int Position = -1]
        {
            get
            {
                var column = Columns.FirstOrDefault(c => c.OwnerAlias == SpecificAlias
                                                         && c.ColumnName == ColumnName
                                                         && (Position < 0 || c.Position == Position));
                if (column == null)
                {
        ...
        /// </summary>
        public bool BestMatchStrict { get; set; } = false;
        ...
        /// <returns></returns>
        public DocRecordColumnInfo GetBestMatch(string Column, string alias = null, int position = -1, bool? strict = null)
        {
            if (strict ?? BestMatchStrict)
        ...
        public void RemoveColumnInfo(IEnumerable<DocRecordColumnInfo> columnsToRemove)
        {
            bool didWork = false;
            int l = Columns.Count;
            foreach (var col in columnsToRemove)
            {
                var check = GetBestMatch(col.ColumnName, col.OwnerAlias, -1, true);
                if (check == null)
                    continue;
                didWork = true;
                int p = Columns.IndexOf(check) + 1;
                for (int i = p; i < l; i++)
                {
                    Columns[i].Position -= 1; //move position down by one for removed columns
                }
        ...
        /// </summary>
        /// <param name="columnsToRemove"></param>
        public void RemoveColumnInfo(params DocRecordColumnInfo[] columnsToRemove)
        {
            RemoveColumnInfo((IEnumerable<DocRecordColumnInfo>)columnsToRemove);
        }
        ...
        /// <summary>
        /// Copy the column to the end of this collection, maintaining original owner alias.
        /// </summary>
