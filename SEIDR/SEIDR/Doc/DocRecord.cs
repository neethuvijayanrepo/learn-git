...
namespace SEIDR.Doc
{
    ...
    public class DocRecord: IRecord
    {
        ...
        public IEnumerable<DocRecordColumnInfo> GetColumns(Predicate<DocRecordColumnInfo> Test)
        {
            return Columns.Where(c => Test(c));
        }
        /// <summary>
        ...
        public DocRecord(DocRecordColumnCollection owner, bool canWrite)            
        {
            Columns = owner;
            CanWrite = canWrite;
            Content = new List<string>(new string[owner.Columns.Count]);
        ...
        public virtual string this[string alias, string ColumnName]
        {
            ...
            set
            {
                if (CanWrite)
                {
                    ...
                    var col = Columns[alias, ColumnName];
                    Content.SetWithExpansion(col.Position, value);
                    if (value != null && !col.TextQualify && Columns.Delimiter.HasValue && value.Contains(Columns.Delimiter.Value))
                    {
                        col.TextQualify = true;
                        Columns.SetFormat();
                    }
