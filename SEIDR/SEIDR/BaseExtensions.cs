namespace SEIDR
{
    ...
    public static class BaseExtensions
    {
        ...
        public static bool ContainsAnySubstring(this string value, bool CaseSensitive, params string[] matches)
        {
            StringComparison comp = CaseSensitive ? StringComparison.Ordinal : StringComparison.OrdinalIgnoreCase;
            foreach (var m in matches)
            {
                if (value.IndexOf(m, comp) >= 0)
                    return true;
            }
            return false;
        }
        ...
        /// <param name="check"></param>
        /// <returns></returns>
        public static bool ContainsAnySubstring(this string value, StringComparison options, params string[] check)
        {
            if (string.IsNullOrEmpty(value))
                return false;
            foreach (var c in check)
            {
                if (value.IndexOf(c, options) >= 0)
                    return true;
            }
            return false;
        }
