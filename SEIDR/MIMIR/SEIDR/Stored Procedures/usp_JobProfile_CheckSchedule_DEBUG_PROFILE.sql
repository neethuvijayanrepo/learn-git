CREATE PROCEDURE [SEIDR].[usp_JobProfile_CheckSchedule_DEBUG_PROFILE]
	@JobProfileID int
AS
BEGIN
	IF @JobProfileID is null
	BEGIN
		RAISERROR('Supply JobProfileID for check.', 16, 1)
		RETURN
	END
	/*
	...
	*/ 
	...
	WHERE jp.ScheduleValid = 1 
	--AND jp.Active = 1  --Not commented out in the actual version of the procedure, but debug version takes a specific profile so does not care about JobProfile.Active
	AND jp.JobProfileID = @JobProfileID
	...
	SELECT 1 [#JobSchedule] WHERE 1=0
	SELECT * 
	FROM #JobSchedule

	SELECT 1 [New History, since Last Job Execution] WHERE 1=0
	...
	SELECT distinct js.JobProfileID, jp.UserKey, jp.UserKey1, jp.UserKey2, 
			1 [StepNumber], 'S' [ExecutionStatusCode],
			[ProcessingDate] = CASE WHEN jp.ScheduleNoHistory =1 then @Now --When profile says no history, use today's date for the ProcessingDate (Date not null).
	...
	SELECT 1 [Today's new JobExecutions] WHERE 1=0
	...
	SELECT js.JobProfileID, UserKey, UserKey1, UserKey2, 
			1 [StepNumber], 'S' [ExecutionStatusCode], [MatchingRuleClusterID],
			@Now [ProcessingDate], @Now [ProcessingTime],js.OrganizationID,js.ProjectID,js.LoadProfileID
	...
	WHERE js.ScheduleValid = 1 
	--AND js.Active = 1
	AND js.JobProfileID = @JobProfileID	
	...
	--AND jp.Active = 1
	AND jp.JobProfileID = @JobProfileID
	AND NOT EXISTS(SELECT null FROM SEIDR.JobExecution WHERE JobProfileID = jp.JobProfileID AND [Manual] = 0 and NotNeeded = 0)
	AND d.Date < @TODAY
	...
	SELECT 1 [Historical - First JobExecutions] WHERE 1=0
	--If not using an interval, may insert multiple days for a single profile here!
	/*INSERT INTO SEIDR.JobExecution(JobProfileID, UserKey, UserKey1, UserKey2,
			StepNumber, ExecutionStatusCode, 
			ProcessingDate, 
	...
	SELECT 1 [Initial Insert for Schedule Matching Today] WHERE 1=0
	...
	SELECT distinct JobProfileID, UserKey, UserKey1, UserKey2, 
			1 [StepNumber], 'S' [ExecutionStatusCode], [MatchingRuleClusterID],
			ProcessingDate = CASE WHEN js.ScheduleNoHistory = 1 then @now else js.ScheduleFromDate end, --Should technically be okay to just do @Now, since the only reason we should need this is due to not including today's date in the temp table (because temp table is for missing days, not for today's executions)
	...
	SELECT 1 [Initial Insert for Schedule using Interval] WHERE 1=0
	...
	SELECT distinct JobProfileID, UserKey, UserKey1, UserKey2, 
			1 [StepNumber], 'S' [ExecutionStatusCode], [MatchingRuleClusterID],
			ProcessingDate = CASE WHEN js.ScheduleNoHistory = 1 then @now else js.ScheduleFromDate end,
			@Now, js.OrganizationID,js.ProjectID,js.LoadProfileID
	FROM SEIDR.JobProfile js	
